  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-xs font-bold text-gray-700 tracking-[0.12em]">{title}</p>
        <p className="text-base font-bold text-gray-900 leading-none">{count}</p>
      </div>
      <p className="text-xs text-gray-500 mt-3 text-right leading-tight group-hover:hidden">{label}</p>
      {onClick && (
        <p className="text-xs text-[#3373AB] mt-3 text-right leading-tight font-medium hidden group-hover:block">{hoverHint || 'Click to manage'}</p>
      )}
    </Comp>
  );
}
